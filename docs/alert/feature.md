...
```
未分派 (unassigned) → 待响应 (pending) → 处理中 (processing) → 已处理/已关闭
                            ↓                ↓
                      自动恢复/自动关闭 (auto_recovery/auto_close)
```
...
*   **智能指纹聚合**：基于 `fingerprint`（MD5 哈希）实现精准去重，相同指纹的活跃告警只会更新而非重复创建
...
>
> *   **图表解读/配置逻辑**：相关性规则决定"哪些事件会被聚合、按什么维度聚合、在多长时间内聚合"。匹配规则控制入口，分组字段控制聚合维度，窗口类型控制时间边界，三者共同定义降噪效果。
