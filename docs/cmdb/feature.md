...
BK-Lite CMDB 围绕「模型管理 → 资产管理 → 发现与同步 → 订阅与通知」四大核心场景，提供全栈式资产配置管理能力。本文档按产品导航模块详细拆解各功能的定位与能力。

---

## 一、模型管理

模型管理是 BK-Lite CMDB 的**数据规则引擎**，决定资产如何被定义、展示和关联。所有实例操作均受模型约束，是确保资产数据一致性的根本。
...
*   **智能显示字段**：用户、组织类型字段自动生成 `_display` 冗余字段，列表页展示中文名称而非 ID
...
*   **关联关系导出**：支持将实例关联关系一并导出，保留结构信息
*   **异步大批量处理**：大数据量导入导出采用异步任务，支持进度查看和结果下载

#### 导入流程

```
下载模板 → 填写数据 → 上传文件 → 系统校验 → 预览结果 → 确认导入 → 查看报告
    ↑                                                            ↓
  格式说明 ←────────────────────  错误修正 ←─────────────────────┘
```
...
资产规模扩大后，**快速定位**成为核心诉求。BK-Lite CMDB 通过「全局视图 + 精准搜索」双入口解决这一问题。
...
| **数据库** | MySQL、PostgreSQL、Redis、MongoDB | Protocol | 实例→库→表（部分）|
| **云平台** | 阿里云、腾讯云 | Protocol | VPC→子网→实例→安全组 |
| **主机** | Linux、Windows | Job | 主机→进程→端口 |
| **中间件** | Nginx、Kafka、Zookeeper 等 | Job | 实例→集群拓扑 |
...
BK-Lite CMDB 不仅采集对象清单，更致力于**恢复对象关系**。
