...
知识管理模块是构建智能问答的基础，负责知识的汇聚、处理与优化。它支持上传文件、录入文本、抓取网页等多种方式汇聚多领域知识，并通过智能算法处理，最终形成可供大模型精准使用的知识库。

该模块核心解决了三个关键问题：

1.  **知识汇聚**：支持上传、抓取、录入多种类型与来源的知识，明确知识库的构成。
2.  **知识处理**：提供多种文本分块策略与参数配置，将非结构化知识转化为易于检索的结构。
3.  **知识检索**：采用**混合检索（语义+文本）** 并结合 **ReRank重排序** 模型，可灵活调整权重，确保从海量知识中快速、准确地定位相关信息。

简而言之，在此模块中，您可完成从知识上传、管理到精细化处理的全流程，为智能体（技能）提供高质量的知识供给。
...
![功能图片38.png](https://static.cwoa.net/90ba770761d84f2dbf4f52a985b31c0e.png)
- **状态**：指示文档当前的处理状态。
    - **错误**：文档在提取、分块或训练过程中失败，需检查文件格式或内容后重新操作。
    - **就绪**：文档已成功处理完成，知识内容已可用，可基于此文档进行问答或检索。
    - **待处理**：文档正在被系统处理，包括文本提取、分块和向量化。

- **提取方式**：选择从上传的文档中提取文本的策略。
    - **全文提取**：将整个文档视为一个整体进行内容提取，适合内容简短、结构单一的文档。
    - **章节提取**：依据文档的标题层级（如H1, H2）自动划分并提取内容，适合结构清晰的长文档。
    - **工作表提取**：专为Excel文件设计，按不同的工作表来提取内容。
    - **行级提取**：按表格的行或固定列宽格式进行内容提取。

- **分块方式**：决定将提取出的文本如何分割成更小的“知识块”以供AI理解。
    - **定长分块**：按固定的字符或Token长度分割文本，可能打断句子或段落。
    - **循环分块**：在定长分块的基础上，让相邻分块之间有部分内容重叠，以保持上下文连贯。
    - **语义分块**：基于文本的语义和自然段落进行分割，尽可能保证每个“块”在语义上的完整性。
    - **不分块**：将提取出的全部文本作为一个整体处理，适用于内容很短的文档。

- **设置**：对文档进行重新处理的入口。选择此操作将重新上传并提取文档内容，适用于文档内容已更新，需要替换知识库中旧版本的情况。

- **训练**：对已提取并分块的文档内容重新进行向量化处理。重新训练后，分块的ID将会改变。
    - **系统会进行二次确认**：
        - **删除关联问答对**：若选择删除，则与此文档所有分块相关联的历史问答对将被清除。
        - **保留问答对**：若选择保留，问答对将得以保存，但它们与原文档分块的关联会被切断，可能影响未来答案的精准溯源。

- **删除**：将源文档从知识库中移除。
    - **系统会进行二次确认**：删除后，该文档对应的所有分块数据将被永久删除，**与之关联的所有问答对也将被一并删除**，此操作不可逆。
...
![功能图片56.png](https://static.cwoa.net/76e22122fc1a4fddb870acb8ebae8858.png)
![功能图片57.png](https://static.cwoa.net/cca41011865a4afaa204dec5725045ce.png)

#### 查看问答对

在知识库中，用户可直观地查看所有问答对。问答对的信息包括名称，问答对数量，创建时间，创建者，操作等，同时可以点击查看每一份问答对拆解的详情问题和答案
![功能图片58.png](https://static.cwoa.net/2b598b5840c84187855327130f8ff552.png)
...
![opspilot功能图片修正76.png](https://static.cwoa.net/eb8345018423421ca0935745ba6640f2.png)
...
对于**知识问答智能体**，聊天增强包括【聊天历史】，【RAG】，【工具】。聊天历史可以帮助机器人理解上下文，并增强回复的相关性和连贯性。RAG开启后先从知识库中检索相关信息，然后使用生成模型产生上下文相关的回答。适用于企业知识库查询，专业问答等。
...
- 节点分为触发方式、应用、智能体、逻辑判断、动作五类，鼠标左键选择节点按钮可拖动至画布内。
- 接口说明：以文档的方式，展示chatflow画布的触发方式。

![chatflow图片1.png](https://static.cwoa.net/a47efed81be84173a5cd10c881b51d93.png)

   **触发方式**

点击画布中的触发方式节点，可进行触发频率、触发时间、触发输入语的设置。点击卡片是节点的编辑，点击绿色按钮是测试执行。
   - 定时触发：按设定的时间规则自动触发流程，适用于定时巡检、定期通知等场景
   - RESTful API：对外提供REST接口，适合以外部系统/应用进行调用
   - OpenAI API：对外提供的接口，可通过OpenAI的方式进行流程调用。
   - AG-UI：通过 AG-UI 页面触发流程，适用于低代码交互场景

   **应用**
   - 嵌入式对话：通过嵌入接口将对话能力集成到第三方系统或页面
   - Web 对话：在 Web 页面中与智能体进行交互
   - 移动端：接收来自 WeOps 服务台移动端的对话请求
   - 企业触发：用于接收企业内部系统或服务台产生的事件触发

   **智能体**
   - 智能体：流程的核心处理节点，用于理解请求并返回处理结果

   **逻辑判断**
   - 条件分支：根据条件判断结果走不同的处理路径
   - 意图分类：根据用户输入意图进行分类处理

   **动作**
   - HTTP 请求：向外部系统发送 HTTP 请求，实现数据交互
   - 对外通知：将处理结果通知到外部系统或渠道


###### ****企业微信应用****

- **参数配置**：需要选择类型填写配置参数，所有参数需在企业微信的后台管理中查询到。
![功能图片86.png](https://static.cwoa.net/e208349edc834feb9dd401e818dd19e5.png)
- **1.**在企业微信中**创建应用**
  - 在网页中登录企业微信后台管理：`https://work.weixin.qq.com/wework_admin`
  - 在应用管理中找到"自建"，创建新的应用
![功能图片87.png](https://static.cwoa.net/87973d8ac7fb4ded979314ddc87de9ec.png)
...
- 1.需要填写参数如下
![功能图片92.png](https://static.cwoa.net/8535703771f24ed7bc636a9d8e18ae60.png)
- 2.创建应用以提供与机器人的交流接口
...
![功能图片94.png](https://static.cwoa.net/a83b0f5c568548428238d301e3e27ddf.png)

###### 公众号

- 1.需要填写参数如下：appid、secret、token、aes key
![功能图片95.png](https://static.cwoa.net/4e242790e8304a8f92a2a6912ed761de.png)
- 2.登录服务号后台（企业公众号）
...
![功能图片.png](https://static.cwoa.net/5b0fb03bb76c467c8521d954aee27e2f.png)
![功能图片.png](https://static.cwoa.net/4b7d46720e8c4102a21970677163669c.png)

##### 接口说明

接口说明模块，核心功能是介绍 ChatFlow（聊天工作流）的触发方式，明确不同触发方式的适用场景与使用逻辑，支撑工作流的自动化启动与外部调用。
![功能图片.png](https://static.cwoa.net/ba64c10f0af54535a3d03df6ae11dd72.png)